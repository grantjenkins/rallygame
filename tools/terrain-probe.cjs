...
for(const [ri,r]of track.ramps.entries()){
 ...
 let airborne=false,land=null,minSpeed=100,postLanding=[];
 for(let i=0;i<7/DT;i++){sim.step({0:sim.ai(c)});const speed=Math.hypot(c.vx,c.vz);if(c.airTime>.15)airborne=true;if(airborne&&c.grounded&&!land)land={pitch:c.pitch,speed,air:c.maxAir,time:sim.time};if(track.delta(c.s,r.s)>0)minSpeed=Math.min(minSpeed,speed);if(land&&sim.time-land.time<1&&i%12===0)postLanding.push({dt:+(sim.time-land.time).toFixed(1),v:+speed.toFixed(1),p:+c.pitch.toFixed(2),lane:+c.lane.toFixed(1),hit:c.hits});}
 console.log('JUMP',ri,{land,minSpeed,gems:c.gems,end:track.delta(c.s,r.s),damage:c.damage,postLanding});
}
