...
button('QA: roof',()=>{const d=window.RallyDebug,c=d.sim.cars[0];c.roll=Math.PI;c.rollV=c.pitchV=0;c.vx=c.vz=c.vy=0;c.y=d.track.height(c.s,c.lane)+1.04;});
button('QA: inspect jump',()=>{const d=window.RallyDebug,c=d.sim.cars[0],r=d.track.ramps[0],p=d.track.at(r.s-24,r.lane);Object.assign(c,{x:p.x,y:p.y+.68,z:p.z,s:r.s-24,lane:r.lane,heading:p.heading,vx:Math.sin(p.heading)*29,vz:Math.cos(p.heading)*29,vy:0});});

button('QA: report',()=>{const d=window.RallyDebug;status.textContent=d.sim.cars.length+' cars / '+(d.ghostReady?'ghost loaded':'no ghost')+' / '+d.phase;});
